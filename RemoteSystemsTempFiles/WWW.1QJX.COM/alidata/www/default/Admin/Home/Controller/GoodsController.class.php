...
class GoodsController extends Controller {
	...
	public function insert() {
        ...
        for($i==0;$i<$data['periods'];$i++){
            ...
            if (false !== $list) {
                //成功提示
                $this->success('商品添加成功',__APP__.'?m=Home&c=Index&a=main');
            } else {
                //错误提示
                $this->error('商品添加失败');
            }
	...
	public function update()
	{
	    ...
	    if (false !== $list) {
	        //成功提示
	        $this->success('商品修改成功',__APP__.'?m=Home&c=Index&a=main');
	    } else {
	        //错误提示
	        $this->error('商品修改失败');
	    }
	...
	public function delete()
	{
	    ...
	    if ($result== 1) {
	        //成功提示
	        $this->success('商品删除成功',__APP__.'?m=Home&c=Index&a=main');
	    } else {
	        //错误提示
	        $this->error('商品删除失败',__APP__.'?m=Home&c=Index&a=main');
	    }
	}
	...
	public function lucky_insert()
	{
	    ...
	    if (false !== $list) {
	        $order=M("Order");
	        $order_info = $order->where("goods_id=$goods_id")->select();
	        //var_dump($order_info);
	        foreach ($order_info as $key=>$val){
	            $lucky_number = unserialize($order_info["$key"]['lucky_number']);
                $isin = in_array($data['lucky_number'],$lucky_number);
                if($isin){
                    $goods1['user_id'] = $order_info["$key"]['user_id'];
                    $goods1['publish_time'] = get_gmtime();
                    M("Goods")->where("id=$goods_id")->save($goods1); // 根据条件更新记录
                }
	            
	        }
	        //成功提示
	        $this->success('幸运号保存成功',__APP__.'?m=Home&c=Index&a=main');
	    } else {
	        //错误提示
	        $this->error('幸运号保存失败');
	    }
