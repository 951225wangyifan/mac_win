...
$(function(){
    ...
    if($("input[name=sc_id]").val()!=''){
    	$.ajax({
    		...
    		success:function(result){
    			...
    			var curpage = $("input[name=curpage]").val();//·ÖÒ³
    	...
    	$.ajax({
    		url:ApiUrl+"/index.php?act=shop&op=shop_list&key=4&page="+pagesize+"&curpage=1"+'&keyword='+$("input[name=keyword]").val(),
    		type:'get',
    		dataType:'json',
    		success:function(result){
    			$("input[name=hasmore]").val(result.hasmore);
	...
	$('.keyorder').click(function(){
		...
		var curkey = $(this).attr('key');//1.ÏúÁ¿ 2.ä¯ÀÀÁ¿ 3.¼Û¸ñ 4.×îÐÂÅÅÐò
		if(curkey == key){
			if(order == 1){
				var curorder = 2;
			}else{
	...
	$('.next-page').click(function(){//ÏÂÒ»Ò³
		var hasmore = $('input[name=hasmore]').val();
		if(hasmore == 'false'){
			return false;
		}
